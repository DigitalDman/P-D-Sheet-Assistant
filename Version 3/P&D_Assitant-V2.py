...
def save():
    ...
    with open(digimon_file_path, "w") as digimon_file: ## Opens/Creates the digimon_sheets.json file, encodes it for Python to interpret
    ...
    with open(pokemon_file_path, "w") as pokemon_file: ## Opens/Creates the pokemon_sheets.json file, encodes it for Python to interpret
...
def build_a_sheet(character_type, build = False): ## Setting a default value makes it an option peramiter
    ...
    if character_type == "Pokemon":
        ...
        while True:
            ...
            print("\t6. Determine move count")

            if build:
                print("\t7.Finish Creating Sheet")
            else:
            ...
            try:
                ...
                ### When an error is triggered. Print the error message & Clear the terminal
                clear_terminal()
                print("ERROR. Selection made is out of range. Please enter a number from 1-7.")
                time.sleep(1)
                clear_terminal()
        ...
        match selection:
            ...
            case 6: ## Determine how many moves the pokemon has
                print(f"{character_used.name} would have {character_used.determine_move()} moves")
                press_enter_to_continue()
                save()
                build_a_sheet(character_type)
