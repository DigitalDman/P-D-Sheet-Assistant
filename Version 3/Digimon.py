...
class Digimon:
    ...
    def save_list(self):
        saved_list = [self.name, self.stats, self.modifiers, self.level, self.hp, self.extra_damage, self.stage_index]
        return saved_list
    ...
    def load(self, save_data ): ## Load some of the stuff (gotta add or rewrite some of this load stuff)
        ## Format of saved list
        ## name, stats, modifiers, level, hp, extra_damage, stage_index
        self.name = save_data[0]
        self.stats = save_data[1]
        self.modifiers = save_data[2]
        self.level = save_data[3]
        self.hp = save_data[4]
        self.extra_damage = save_data[5]
        self.stage_index = save_data[6]
        #self.stage = stage
    ...
    def increase_health(self, digivolve, times_increased):
        ...
        if user == "y":
            ...
            for i in range(times_increased - 1):
                new_hp_stat += brawn_dice // 2 ## Floor divide

        else:
            new_hp_stat = self.hp + brawn_dice // 4 ## Floor divide

            for i in range(times_increased - 1):
                            new_hp_stat += brawn_dice // 4 ## Floor divide
    ...
    def select_stage(self):
        ...
        while True:
            ...
            try:
                ...
                ### When an error is triggered. Print the error message & Clear the terminal
                self.clear_terminal()
                print("ERROR. Selection made is out of range. Please enter a number from 1-4.")
                time.sleep(1)
                self.clear_terminal()
    ...
    def level_up(self):
        ...
        new_max_hp = self.hp
        plus_1s_gained = 0
        extra_damage_gained = 0
        new_modifiers = self.modifiers
        new_hp_stat = 0
        
        #### Figure out how many times your leveling up
        while True:
            print("How many times do you want to level up?")
            print("Max Level - 30")
            print(f"Current Level - {self.level}")

            try:
                level_ups = int(input("Input: "))
                if level_ups < 1 or (level_ups > 30 - self.level):
                    print(5/0)
        ...
        ## Level up now ##
        self.clear_terminal()
        print(f"Increasing {self.name}'s hp")
        new_hp_stat = self.increase_health(False, level_ups) ## increase hp

        time.sleep(1)

        self.clear_terminal()
        
        ## +1’s you should have, - +1’s you do have, = +1’s needed
        plus_1s_gained = ( (self.level + level_ups) // 3 ) - ( self.level // 3 )
        ...
        ## Same math idea as how many +1’s you need
        ## Extra damage you should have - Extra damage you need 
        extra_damage_gained = ( (self.level + level_ups) // 4 ) - ( self.level // 4 )
        ...
        if self.do_you_accept_changes():
            ...
            self.level = self.level + level_ups
            self.modifiers = new_modifiers
    ...
    def proper_display(self):
            print(self.name)
            print(f"Level - {self.level}       Stage - {self.stage[self.stage_index]}       HP - {self.hp}")
